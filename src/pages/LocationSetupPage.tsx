...
export const LocationSetupPage: FC = () => {
  ...
  const [locationMode, setLocationMode] = useState<'automatic' | 'manual'>(savedLocation?.mode || 'automatic');
  const [randomizationRadius, setRandomizationRadius] = useState<number>(savedLocation?.randomizationRadius || 0);
  const [manualLocation, setManualLocation] = useState<string>(savedLocation?.address || '');
  ...
  const [selectedCoordinates, setSelectedCoordinates] = useState<{ lat: number; lng: number } | null>(
    savedLocation ? { lat: savedLocation.latitude, lng: savedLocation.longitude } : null
  );
  ...
  useEffect(() => {
    if (savedLocation) {
      setLocationMode(savedLocation.mode);
      setRandomizationRadius(savedLocation.randomizationRadius || 0);
      setManualLocation(savedLocation.address || '');
      setSelectedCoordinates({ lat: savedLocation.latitude, lng: savedLocation.longitude });

      // If it's automatic mode, set the automatic location to the saved coordinates
      if (savedLocation.mode === 'automatic') {
        setAutomaticLocation({ lat: savedLocation.latitude, lng: savedLocation.longitude });
      }
    }
  }, [savedLocation]);
  ...
  const handleNextPageClick = async () => {
    if (selectedCoordinates) {
      const locationData: Location = {
        ...
        mode: locationMode,
        randomizationRadius: locationMode === 'automatic' ? randomizationRadius : undefined,
        address: locationMode === 'manual' ? manualLocation : undefined
      };
